...
def get_status():
    ...
    table = soup.find('div',{'class':'tbl'}) 
    tbody = table.find_all('tbody')
    columns = [column.text for column in tbody[1].select("td")]
    
    # 정인 : columns 안에 순서 다음과 같음 
    # 확진환자	접촉자	     검사 중	         결과 음성
    #                     입원	  일반
    ...
    # 정인 : 인천은 완치자 수를 제공하지 않아서 ['확진자 수' , '접촉자 수'] 로 크롤링 하였음 
...
def get_patients():
    ...
    patient_profile = soup.find_all("div",{"class":"patient-profile-route-group"})
    total_patients = int(len(patient_profile))
    patients = {}
    patient_num = 1

        
    for row in patient_profile:  
        patinet_detail = row.select('.patinet-profile')
        patinet_path = row.select('li')
        #print(patinet_detail[0])

        patients[patient_num] = {}
        patients[patient_num]["Paths"] = [path.text.replace("\n","") for path in patinet_path] 

        # informations = [info.text for info in patinet_detail.select('strong')]
        # print(informations)
        for detail in patinet_detail:

            informations = [info.strip() for info in detail.get_text().split("\n")]
            patients[patient_num]["ID"] = (informations[1] if informations[1] != '' else "미정")
            patients[patient_num]["Region"] = (informations[2] if informations[2] != '' else "미정")

            informations_detail = informations[3].split("/")
            gender_age = informations_detail[0]
            patients[patient_num]["Gender"] = gender_age[1:2]
            patients[patient_num]["Age"] = gender_age[2:]
           # 인천 홈페이지 날짜 형식 통일 안 해놔서 내가 함 ㅡㅡ 
            confirmed_date = informations_detail[1].strip()
            if len(confirmed_date)>10 :
                if len(confirmed_date)<15:
                    confirmed_date = informations_detail[1].strip()[:-3]
                else :
                    confirmed_date = informations_detail[1].strip()[:-6]
            confirmed_date = confirmed_date.split(".")

            patients[patient_num]["Confirmed Date"] = ".".join(confirmed_date[:3])
            patients[patient_num]["Current Status"] = (informations_detail[2])[:-1] #인천은 현재 상태가 아니라 감염경로
    ...
    patients = {
        1: {
            ...
            “Paths”: raw 환자 동선 정보,  !!! -> 홈페이지에 따라 확진경로로 변경하였음 
        },
        2: {
            “ID”: 환자 식별자,
            “Gender”: 환자 성별,
